...
class AWSDockerDeployer:
    ...
    def _create_vpc_and_subnets(self) -> Dict[str, str]:
        """Create VPC, Internt Gateway and public and private subnets within Availability zones.
        Connects the subnets and Gateway through route tables.
        """
        ...
        if vpc:
            ...
            return {
                ...
                'subnet_ids': subnet_ids, # ['subnet-0a0e5aae628467d84', 'subnet-0277198dc6e67f726']
            }
    ...
    def _create_alb_security_groups(self, vpc_id:str) -> str:
        """Creates Security Groups for the LoadBalancer
        Args:
            vpc_id (str): _description_
        Returns:
            str: Load Balancer security group Id
        """
    ...
    def _create_ecs_security_groups(self, vpc_id:str, alb_sec_group_id:str) -> str:
        """Creates, if one does not exists yet, a security group for ECS. Uses Cluster Name to identify the security group.
        Args:
            vpc_id (str): _description_
            alb_sec_group_id (str): _description_

        Returns:
            str: ECS security group Id
        """
    ...
    def _create_cloudwatch_log_groups(self, services: list, svc_log_prefix: str):
        """Create CloudWatch log groups for services"""
        for service in services:
            log_group_name = f'{svc_log_prefix}/{service}'
            try:
    ...
    def _create_task_definition(self, service_name: str, image_uri: str, port: int, 
                             ...
                             svc_log_prefix:str, env_values: Optional[dict] = None) -> str:
            ...
            'containerDefinitions': [
                {
                    ...
                    'logConfiguration': {
                        'logDriver': 'awslogs',
                        'options': {
                            'awslogs-group': f"{svc_log_prefix}/{service_name}",
    ...
    def _get_ecs_task_public_ip(self, service_name:str, deploy_svc_rev:str) -> str : 
        ...
        try : 
            print(f"Retrieving service public IP using Task service Deployment version {deploy_svc_rev}")
            tasks_list = self.ecs.list_tasks(
                cluster=self.cluster_name,
                # serviceName=service_name,
                startedBy=deploy_svc_rev,
    ...
    def deploy_infrastructure(self, repo_uris: Dict[str, str]):
        ...
        #Give time for the above App ECS service to be deployed and started to get right IP addr
        #TODO : A more elegant solution ? Check if abv svc is up and running ?
        print("Waiting for App task service to be deployed and started.....")
        time.sleep(30)
        # get info abt latest deployment 
        app_deployment_svc_rev = app_ecs_response['service']['deployments']['status'=='PRIMARY']['id']

        
        # create env vars
        public_ip = self._get_ecs_task_public_ip(service_name=app_service_name, deploy_svc_rev=app_deployment_svc_rev)
        ...
        #TODO : Load Balancer's Target Group(s) need to be updated with Container / Service Task's Public IP address 

        print("\nDeployment complete!")
        print(f'''BUT ***** Please add Service Task's Public IP addr to Target Group on the LoadBalancer {alb_info}.
                Until this step is complete, below URLs would not work ******''')
