...
from featurepipe.datalogic.embedding_data_handlers import EmbeddingHandlerFactory
...
class ChunkingDispatcher:
    ...
    def dispatch_chunker(cls, data_model: DataModel) -> list[DataModel]:
        ...
        logger.info("Data chunked successfully ", data_type=data_type, chunked_content_len=len(chunk_models))
...
class EmbeddingDispatcher:
    ...
    def dispatch_embedder(cls, data_model:DataModel) -> DataModel:
        data_type = data_model.type
        handler = cls.embedding_factory.create_handler(data_type=data_type)
        embedded_chunk_model = handler.embed(data_model=data_model)
